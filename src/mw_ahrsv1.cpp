...
public:
  ...
  void publishTopic(unsigned char* packet)
  {
    ...
    tf2::convert(quat_tf, orientation);
...
int main(int argc, char* argv[]) {
  ...
  rclcpp::Node::SharedPtr node = rclcpp::Node::make_shared("mw_ahrsv1",
  ...
  MwAhrsV1ForROS sensor(node, serial_port, baud_rate);

  if(sensor.initialize() == false) {
    RCLCPP_ERROR(node->get_logger(), "Initialize() returns false, please check your devices.");
    RCLCPP_ERROR(node->get_logger(), "Set port parameters using the following Linux command:\n stty -F /dev/ttyUSB? 115200 raw");
    RCLCPP_ERROR(node->get_logger(), "You may need to have ROOT access");
    return 0;
  } else {
    RCLCPP_INFO(node->get_logger(), "MW-AHRSv1 Initialization OK!");
  }
